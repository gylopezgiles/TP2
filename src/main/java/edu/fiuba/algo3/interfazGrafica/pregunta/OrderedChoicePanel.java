...
public class OrderedChoicePanel extends JPanel implements JPanelPregunta{
    ...
    public OrderedChoicePanel(List<Opcion> opciones){
        JList<Button> opcionesEnPantalla = new JList<>(new DefaultListModel<>());
        ...
        opciones.stream().forEach(opcion -> agregarOpcion(opcion, opcionesEnPantalla));
        Box botones = Box.createVerticalBox();
        botones.add(opcionesEnPantalla);
        add(botones);

    }

    private void agregarOpcion(Opcion opcion, JList<Button> opcionesEnPantalla) {
        JButton opcionButton = new JButton();
        opcionButton.setText(opcion.obtenerTexto());
        opcionesEnPantalla.add(opcionButton);
        //opcionesButtonGroup.add(opcionButton);
    }
